...
for filename in os.listdir('input'):
    ...
    for move in dancedata:
        ...
        if move['GoldMove'] == 1 and '.msm' in move['ClassifierPath']: # making Gold Move Effect
            GoldEffectClip = {"__class":"GoldEffectClip","IsActive":1,"Duration":24,"EffectType":0}
            GoldEffectClip['Id'] = randint(1000000000, 9999999999) # random
            GoldEffectClip['TrackId'] = randint(1000000000, 9999999999)
            GoldEffectClip['StartTime'] = move['StartTime'] + 12 # were adding 12 to the starttime unless it will come too early
            tape['Clips'].append(GoldEffectClip)
        if f'world/maps/{MapName.lower()}/timeline/moves/' not in move['ClassifierPath']:
            newmove = f'world/maps/{MapName.lower()}/timeline/moves/{move["ClassifierPath"]}'
            move['ClassifierPath'] = newmove
        tape['Clips'].append(move)

    pictoclips = mainjson['m_danceData']['PictoClips']
    for picto in pictoclips:
